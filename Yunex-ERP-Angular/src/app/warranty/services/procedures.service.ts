...
export class ProceduresService {
  ...
  getAllFiles():Observable<any>{
    const url = environment.API_URI +"warrantyProcedure/get-all";
   ...
   */
  createFile(file:any){
    const url = environment.API_URI+"warrantyProcedure/create-new";
  ...
  dowloadFile(name:string):Observable<any>{
    const url = environment.API_URI+"warrantyProcedure/get";
  ...
  updateFile(id:string,file:any):Observable<any>{
    const url = environment.API_URI+"warrantyProcedure/update";
  ...
  async deleteFile(name:string):Promise<boolean>{
    const url = environment.API_URI+"warrantyProcedure/delete";
    try {
      const res = await this.http.delete(url,{ body: { name: name } }).toPromise()
      const response = JSON.parse(JSON.stringify(res));
      return response.msg === 'WarrantyProcedure deleted correctly' ? true : false;
    } catch (error) {
